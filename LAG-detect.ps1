#detect script: https://www.petervanderwoude.nl/post/detecting-local-administrators-with-proactive-remediations/
#Define variables
$localAdministrators = @()
$memberCount = 0
    ...
    if ($localAdministrators.Count -eq $numberLocalAdministrators) {
        foreach($localAdministrator in $localAdministrators) { 
            switch ($localAdministrator) { 
                ...
                # If you are using local additional groups, include them here
